...
def container_action(container_id: str, body: ContainerAction, _: User = Depends(get_current_user)):
    _guard()
    try:
...
def run_container(body: RunContainer, _: User = Depends(get_current_user)):
    _guard()
